...
const backendURL = "https://back-8.onrender.com/api/stock-in";
...
function StockIn() {
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      ...
      setStockInList(
        stockInList.map((entry) => (entry._id === editingId ? res.data : entry))
      );
      setEditingId(null);
      setEditForm({});
    } catch (err) {
  ...
  return (
    <div className="bg-white p-6 rounded-xl shadow text-gray-800 max-w-6xl mx-auto">
      ...
      <form onSubmit={handleAddEntry} className="grid md:grid-cols-3 gap-4 mb-6">
        ...
        <input name="quantity" type="number" value={form.quantity} onChange={handleChange} placeholder="Quantity" className="p-2 border rounded" required min="1" />
        ...
        <button type="submit" className="bg-blue-900 text-white rounded px-4 py-2 hover:bg-blue-800 col-span-1">Add Entry</button>
      </form>

      <div className="flex flex-wrap gap-4 mb-4 items-center">
        <input type="text" placeholder="Search by product or supplier..." value={search} onChange={(e) => setSearch(e.target.value)} className="p-1 px-2 border rounded text-sm w-64" />
        <label>From: <input type="date" value={dateFrom} onChange={(e) => setDateFrom(e.target.value)} className="border rounded p-1" /></label>
        <label>To: <input type="date" value={dateTo} onChange={(e) => setDateTo(e.target.value)} className="border rounded p-1" /></label>
        <button onClick={() => { setSearch(""); setDateFrom(""); setDateTo(""); }} className="text-sm text-red-600 hover:underline">Clear Filters</button>
      </div>
            ...
            {paginated.length > 0 ? (
              paginated.map((entry) => (
                <tr key={entry._id} className="border-t hover:bg-gray-50">
                  {editingId === entry._id ? (
                    <>
                      <td><input className="border p-1 rounded" name="productName" value={editForm.productName} onChange={handleEditChange} /></td>
                      <td><input className="border p-1 rounded" name="quantity" type="number" min="1" value={editForm.quantity} onChange={handleEditChange} /></td>
                      <td><input className="border p-1 rounded" name="supplier" value={editForm.supplier} onChange={handleEditChange} /></td>
                      <td><input className="border p-1 rounded" name="date" type="date" value={editForm.date} onChange={handleEditChange} /></td>
                      <td><input className="border p-1 rounded" name="notes" value={editForm.notes} onChange={handleEditChange} /></td>
                    ...
                    <>
                      ...
                      <td className="p-2 space-x-2">
                        <button onClick={() => { setEditingId(entry._id); setEditForm(entry); }} className="text-blue-600 hover:underline">Edit</button>
              ...
              <tr><td colSpan="6" className="p-4 text-center text-gray-500">No stock in records found.</td></tr>
            )}
          </tbody>
        </table>
      </div>
      ...
      <div className="flex justify-between mt-4">
        ...
        <span className="text-gray-600">Page {page} of {Math.ceil(stockInList.length / pageSize)}</span>
        <button disabled={page * pageSize >= stockInList.length} onClick={() => setPage(page + 1)} className="text-blue-700 hover:underline">Next</button>
      </div>

      <div className="mt-4 space-x-4">
        <CSVLink data={csvData} filename="stock_in_report.csv" className="text-indigo-700 hover:underline">Export CSV</CSVLink>
        <button onClick={printReport} className="text-green-700 hover:underline">Print PDF</button>
      </div>
    </div>
  );
}
